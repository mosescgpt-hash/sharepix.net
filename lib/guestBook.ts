/**
 * The guest book: signed notes guests leave alongside the photos they upload.
 *
 * Everything here is a pure function so it can be tested without AWS, and so
 * the browser and the Lambda agree on the same rules. The Lambda cannot import
 * this file (Amplify functions bundle separately), so
 * `amplify/functions/create-guest-book-entry/entry.ts` carries a copy — the
 * duplication is deliberate and each side has its own tests.
 *
 * The browser's copy of these rules is a courtesy to the guest, not a control.
 * Every one of them is re-applied server-side, because a guest is
 * unauthenticated and nothing they send can be trusted.
 */
