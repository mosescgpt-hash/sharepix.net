/**
 * The monthly report: how did SharePix do last month, from data we actually
 * have.
 *
 * ## The one rule
 *
 * **Report what is measured. Say what is not. Never print a zero for
 * something nobody counts.**
 *
 * The brief for this report assumes a central analytics system tracking
 * website traffic, a purchase funnel, refunds, unit economics, experiments,
 * visitor intent, referrals, support burden and a dozen other things. Almost
 ...
 *
 * So the sections here are the ones with real numbers behind them, and
 * `NOT_MEASURED` names the rest explicitly, in the email, every month. That
 * list shrinking is itself a useful signal.
 *
 * ## Month-over-month
 *
 * Every figure is shown against the month before, because a number on its own
 * is nearly useless — "9 events" means nothing until you know last month was
 * 3 or 30. Deltas are omitted, not shown as +100%, when the previous month had
 * nothing to compare against: a percentage change from zero is a division we
 * would be inventing.
 */
