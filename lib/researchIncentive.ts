/**
 * The $25 Amazon gift card for completing the post-event research survey.
 *
 * Manual by design. Nothing here sends a gift card, and nothing here can mark
 * one sent — a person does both. At the volumes this programme runs at, someone
 * buying and emailing a card is cheaper and far less risky than an automated
 * payout path, which would be a new class of fraud surface on a product whose
 * only current money movement is inbound through Stripe.
 *
 * ## The rule that matters most
 *
 * **Eligibility may never depend on what the feedback said.** Not on a rating,
 * a satisfaction score, a willingness to recommend, a testimonial, or a
 * permission to use their photos. Someone who tells us the product was a
 * disappointment earns exactly the same $25 as someone who loved it.
 *
 * This is not politeness. Research paid for on condition of approval is not
 * research — it buys agreement and then reports it as evidence, and every
 * decision made on it afterwards is made on a lie we told ourselves. The rule
 * is enforced here rather than left to the copy: `eligibilityFor` is given no
 * access to answers at all, so a future change cannot quietly start reading
 * them without someone deleting this comment first.
 */
