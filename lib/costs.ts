/**
 * What SharePix costs to run, and how well each figure is actually known.
 *
 * ## The question this answers
 *
 * "How much money do I need in the bank this month." Not "what did I spend" —
 * that is a bookkeeping question and the answer arrives after it is useful.
 *
 * Two things follow from that. The headline is a **projection to month end**,
 * because a bill you have already paid is not the one to prepare for. And
 * pass-through costs are excluded from it: Prodigi's print bill and Stripe's
 * fees are covered by the buyer at the point of sale (see `prints.ts`), so
 * counting them as money to have ready would inflate the number by the one part
 * that funds itself.
 *
 * ## Why every figure carries its provenance
 *
 * A total is only as trustworthy as its worst line, and these lines are not
 * equally trustworthy:
 *
 *   - **measured** — came from the provider's own API. AWS Cost Explorer and
 *     Stripe balance transactions are the actual bill and the actual fee.
 *   - **computed** — derived from data SharePix owns, using rates recorded
 *     here. Right as long as the rates are.
 *   - **declared** — somebody typed it in. Right on the day they typed it.
 *   - **free** — no charge at this volume, listed so the absence is deliberate
 *     rather than an oversight.
 *
 * The page shows the split rather than one confident total. This repository has
 * a long history of numbers that were true when written and quietly stopped
 * being true — the README has a section about it — and a costs page is the
 * worst place to repeat it, because a wrong total here looks exactly like a
 * right one.
 *
 * That is also why a declared figure ages. `DECLARED_STALE_AFTER_DAYS` is not a
 * rule about accounting, it is an admission that the person who typed $6 in
 * January has no idea whether it is still $6.
 */
