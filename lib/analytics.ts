/**
 * The funnel: what SharePix records, and what it refuses to guess.
 *
 * SharePix has no analytics vendor. Cloudflare Web Analytics counts page views
 * and cannot join one to a purchase, which is why lib/monthlyReport.ts has been
 * carrying "Checkout starts, and the pricing-to-purchase funnel" in its
 * NOT_MEASURED list. This module is the first-party substitute, and it is
 * deliberately provider-agnostic: the names below are the vocabulary, and where
 * they are written is a detail behind `recordEvent`.
 *
 * ## The principle the funnel is built on
 *
 * A sale is not a successful event. Someone paying for SharePix has not yet
 * received anything; the product has delivered when a person who is *not the
 * host* uploads through the QR code. So `purchase_completed` is a middle stage
 * and `first_guest_upload` is the one that matters — see FUNNEL_STAGES, where
 * activation sits at six of eight rather than at the sale.
 *
 * ## Truth, and where it comes from
 *
 * Events are split by how much they can be trusted. Anything derived from a row
 * SharePix maintains server-side — an event created, a payment recorded, a
 * contributor counted — is a fact. Anything a browser reports is a claim: it
 * can be missed by an ad blocker, doubled by a reload, or fabricated. Both are
 * useful and they are not the same thing, so `SERVER_TRUTH` names which is
 * which and the dashboard says so rather than presenting one number.
 */
