/**
 * Featured Events: how a customer gives SharePix the right to use specific
 * photos, and what that right does and does not cover.
 *
 * ## The rule everything here exists to enforce
 *
 * A customer's photos are theirs. Buying SharePix is not a licence, answering a
 * survey is not a licence, and saying "yes, contact me" is not a licence. The
 * only thing that grants one is a person choosing specific assets and accepting
 * a named release, and this module is what records that.
 *
 * The survey's `photoMarketingInterest` question is the invitation, not the
 * grant — lib/survey.ts says so beside the question. This is where the grant
 * actually happens, per asset, with the release version and the moment stamped
 * on it, so a licence obtained in 2026 still means what its wording said in
 * 2026 and not what a later version says.
 *
 * ## What a release cannot do
 *
 * It cannot bind the people in the photograph. A host can grant what is theirs
 * to grant and no more, which is why REVIEW_CHECKS exists and why a submission
 * is reviewed by a person before anything is used. A contract can allocate
 * responsibility and obtain warranties; it cannot make a stranger in the
 * background of a wedding photo into a consenting party.
 *
 * ## Money
 *
 * Nothing in this file moves money, and nothing anywhere in this codebase does
 * — the same rule the refund ledger keeps (lib/refunds.ts). Compensation is a
 * queue item a person acts on, and `compensationState` is a record of what was
 * decided, never an instruction that pays.
 */
