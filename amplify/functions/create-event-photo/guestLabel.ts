...
/**
 * A stable per-browser, per-event key, minted and stored on first use.
 *
 * Deliberately NOT the label. The label is four characters of entropy and is
 * shown to other people; this is neither. It exists only so the same browser
 * tapping like twice is counted once, and it is not a credential — the server
 * treats it as a hint about which browser, never as proof of who.
 *
 * Same storage discipline as the label: every access is wrapped, and a browser
 * with storage locked down still gets a working key for this page load. The
 * consequence there is that a like may not survive a refresh, which is a far
 * smaller cost than refusing the tap.
 */
const KEY_BYTES = 16;

function guestKeyStorageKey(eventId: string): string {
  return `sharepix-guest-key-${eventId}`;
}

/** Whether a stored value still looks like one of ours. Matches the server's rule. */
export function isGuestKey(value: string | null | undefined): boolean {
...
function mintGuestKey(): string {
  ...
  try {
    ...
    // No crypto (very old browser, or a non-secure context). Math.random is
    // weak, and that is acceptable here precisely because this is not a
    // credential: the worst case is that two browsers collide and one like is
    // miscounted on a gallery of a party.
    for (let i = 0; i < KEY_BYTES; i += 1) bytes[i] = Math.floor(Math.random() * 256);
  }
  let out = '';
...
export function guestKeyFor(eventId: string): string {
  const key = guestKeyStorageKey(eventId);
  try {
    const stored = window.localStorage.getItem(key);
    if (isGuestKey(stored)) return stored as string;
  } catch {
    return mintGuestKey();
  }
  const minted = mintGuestKey();
  try {
