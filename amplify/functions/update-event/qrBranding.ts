/**
 * The QR branding rules, as the Lambda sees them.
 *
 * A byte-for-byte copy of the code in `lib/qrBranding.ts`. Amplify functions
 * bundle separately and cannot import from the app, so the duplication is
 * forced; `__tests__/qr-branding-function-copy.test.ts` fails if the two ever
 * drift. Edit `lib/qrBranding.ts` and re-copy.
 *
 * This is the SERVER's copy, and the only one that decides. The browser runs
 * the same checks so the host is told immediately, but a logo that is too big
 * or a colour that will not scan is refused here.
 */
