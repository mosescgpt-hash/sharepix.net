/**
 * The Lambda's copy of the survey definitions.
 *
 * Byte-identical to lib/survey.ts below the header; the drift guard is in
 * __tests__/survey-function-copy.test.ts. The daily job needs only the timing
 * rules from it — surveyIsDue and reminderIsDue — but takes the whole module
 * rather than a hand-picked subset, because a subset is a third definition and
