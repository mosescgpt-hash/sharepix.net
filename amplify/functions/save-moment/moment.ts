/**
 * The moment rules, as the Lambda sees them.
 *
 * A byte-for-byte copy of the code in `lib/moments.ts`. Amplify functions are
 * bundled separately and cannot import from the app, so the duplication is
 * forced; `__tests__/moments-function-copy.test.ts` fails if the two ever
 * drift, which is the part that actually matters. Edit `lib/moments.ts` and
 * re-copy.
 *
 * This is the SERVER's copy, and the only one that is a control. The browser's
 * identical checks are a courtesy to the host.
 */
